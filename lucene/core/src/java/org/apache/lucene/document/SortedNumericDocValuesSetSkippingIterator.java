...
/**
 * Wrapper around a {@link TwoPhaseIterator} for a sorted numeric doc-values set query that speeds
 * things up by taking advantage of a {@link DocValuesSkipper}.
 */
final class SortedNumericDocValuesSetSkippingIterator extends TwoPhaseIterator {
  ...
  SortedNumericDocValuesSetSkippingIterator(
      TwoPhaseIterator twoPhase, DocValuesSkipper skipper, DocValuesLongHashSet numbers) {
    super(new Approximation(twoPhase.approximation(), skipper, numbers));
  ...
  static class Approximation extends DocIdSetIterator {
    ...
    Approximation(
        DocIdSetIterator innerApproximation, DocValuesSkipper skipper, DocValuesLongHashSet numbers) {
    ...
    private Match match(int level) {
      ...
      if (numbers.intersects(minValue, maxValue) == false) {
        return Match.NO;
